...
- Living Character Spoken Dialogue Contract: Enforced mandate requiring exactly one dialogue block alongside prose when companions or ambient personnel are present; authorizes both directly addressed dialogue (`COMMUNICATE`) and spontaneous companion speech during observation turns (`OBSERVE`), while strictly preventing hallucinations.
- Speaker Identification & Prefix Stripping Normalization: In `validateDialogueBlocks` and `resolveDialogueSpeakerId`, matches on both character ID and display name, normalizes speaker to display name, and strips duplicate `Name: ` prefixes emitted in content.
