...
- **Self-Preservation & Fear Dynamics (Horror Grammar 3)**: A deterministic salience ledger tracks acute terror (`spike`), background dread (`dread`), and threat vectors (`life`, `freedom`, `identity`) with exponential decay and residue retention. High fear shifts NPC behavior into survival instincts (`FLEE`, `HIDE`, `SUBMIT`), triggers prey mode with hysteresis, and emits 4-band somatic tokens (`[SOMATIC STATE: ...]`). Player sovereignty guarantees human player actions are never overridden while somatic state reflects physical trauma.
