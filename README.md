...
- **Ad-Lib Induction:** define a scenario directly through a compact set of authored details without first creating a complete Blueprint.

Both paths converge on the same stateful turn pipeline. Ad-Lib is not a disposable demo path. It is a looser authoring surface that can materialize details as they become relevant while still obeying the same canonical state, geometry, memory, ratification, and telemetry rules as a designed scenario.
...
- 30 targeted tests across the Ad-Lib induction and Phase 2F behavioral suites;
- 182 tests across 25 Vitest files;
