...
export const useTelemetryStore = create<TelemetryState>((set, get) => ({
  ...
  getMomentumIndex: () => {
    ...
    if (!rollingWindow || rollingWindow.length === 0) return 0.5;
    ...
    const avgUrgency =
      rollingWindow.reduce((sum, t) => sum + (t.semanticUrgency || 0), 0) / divisor;
    const avgSanityLoss =
      rollingWindow.reduce((sum, t) => sum + Math.abs(t.sanityDelta || 0), 0) / divisor;
    ...
    return isNaN(momentum) ? 0.5 : parseFloat(momentum.toFixed(2));
  },
}));
