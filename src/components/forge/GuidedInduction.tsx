...
export const GuidedInduction: React.FC = () => {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const handleUpdateTerminal = (vector: 'somaticTerminal' | 'narrativeConvergence' | 'cognitiveCollapse', fields: any) => {
    setBlueprintDraft(prev => ({
      ...prev,
      terminalConditions: {
      ...
      <div className="col-span-5 border border-zinc-800 bg-zinc-950/40 p-6 flex flex-col justify-between rounded">
        <div>
          ...
          {step === 1 && (
            <div className="space-y-4">
              <h3 className="text-sm font-bold text-zinc-100 uppercase tracking-wider">Phase 1: Thematic Anchor</h3>
              <p className="text-xs text-zinc-500 leading-relaxed">Define the inescapable psychological truth or prime vector that governs this scenario loop.</p>
              <div>
              ...
              <div>
                <label className="text-[10px] text-zinc-400 block mb-1">THEMATIC ANCHOR (THE SYSTEM TRUTH)</label>
                <textarea 
          ...
          {step === 2 && (
            <div className="space-y-4">
              <h3 className="text-sm font-bold text-zinc-100 uppercase tracking-wider">Phase 2: Somatic Boundaries</h3>
              <p className="text-xs text-zinc-500 leading-relaxed">Establish the strict conditions under which the protagonist's physical shell systemically fails.</p>
              <div>
                <label className="text-[10px] text-zinc-400 block mb-1">FATAL DEBUFF TAGS (Comma Separated)</label>
                <input 
                  type="text" 
                  placeholder="e.g., concussed_unconscious, hemorrhaging, shattered_spine"
                  className="w-full bg-zinc-900 border border-zinc-800 p-2 text-sm focus:outline-none focus:border-zinc-600 font-mono text-zinc-200"
                  onChange={(e) => handleUpdateTerminal('somaticTerminal', { fatalThresholdTags: e.target.value.split(',').map(s => s.trim()) })}
                />
              </div>
              <div>
                <label className="text-[10px] text-zinc-400 block mb-1">SOMATIC TERMINAL RESOLUTION PROSE</label>
                <textarea 
                  className="w-full bg-zinc-900 border border-zinc-800 p-2 text-sm focus:outline-none focus:border-zinc-600 h-24 font-mono text-zinc-200 resize-none"
                  placeholder="Describe the cold cessation of the system when physical capacity drops to zero."
                  onChange={(e) => handleUpdateTerminal('somaticTerminal', { narrativeResolution: e.target.value })}
                />
              </div>
            </div>
          )}
          ...
          {step === 3 && (
            <div className="space-y-4">
              <h3 className="text-sm font-bold text-zinc-100 uppercase tracking-wider">Phase 3: Narrative Convergence</h3>
              <p className="text-xs text-zinc-500 leading-relaxed">Define the strict, pyrrhic conditions that allow survival at an absolute semantic cost.</p>
              <div>
                <label className="text-[10px] text-zinc-400 block mb-1">REQUIRED CONVERGENCE FLAGS (Comma Separated)</label>
                <input 
                  type="text" 
                  placeholder="e.g., sacrifice_accepted, anchor_severed"
                  className="w-full bg-zinc-900 border border-zinc-800 p-2 text-sm focus:outline-none focus:border-zinc-600 font-mono text-zinc-200"
                  onChange={(e) => handleUpdateTerminal('narrativeConvergence', { requiredStateFlags: e.target.value.split(',').map(s => s.trim()) })}
                />
              </div>
              <div>
                <label className="text-[10px] text-zinc-400 block mb-1">PYRRHIC CLOSURE PROSE</label>
                <textarea 
                  className="w-full bg-zinc-900 border border-zinc-800 p-2 text-sm focus:outline-none focus:border-zinc-600 h-24 font-mono text-zinc-200 resize-none"
                  placeholder="Describe the state of reality-neutralization once the loop satisfies its cost."
                  onChange={(e) => handleUpdateTerminal('narrativeConvergence', { resolutionSequence: e.target.value })}
                />
              </div>
            </div>
          )}
        ...
        <div className="flex justify-between items-center pt-4 border-t border-zinc-900 mt-6">
          ...
          {step < 3 ? (
            ...
            <button 
              onClick={() => console.log("[FORGE COMPILER] Compiling Guided Blueprint State...", blueprintDraft)}
